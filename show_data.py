...
# --- CRITICAL FIX 1: set_page_config MUST be the first Streamlit command ---
...
# --- MOBILE COLUMN CSS FORCE ---
# This injects CSS that forces ONLY your inner image/text layout to stay side-by-side on mobile.
st.html(
    """
    <style>
        /* Target only the specific columns inside our metric container */
        [data-testid="stHorizontalBlock"]:has(img) {
            flex-direction: row !important;
            flex-wrap: nowrap !important;
        }
        [data-testid="stHorizontalBlock"]:has(img) > div {
...
rank_pie.update_traces(
    marker_colors=tier_colors, textinfo="percent+label", textposition="inside"
)
...
with col3:
    ...
    with st.container():
        ...
        with text_col:
            # --- CRITICAL FIX 2: Removed non-existent 'height' argument ---
            st.metric(
                label="Total Gadgets", value=f"{gadgets}/{len(brawlers)*2}"
            )
            st.metric(
                label="Total Starpowers", value=f"{starpowers}/{len(brawlers)*2}"
            )
            st.metric(
                label="Total Hypers",
                value=f"{len(brawlers) - hypers_missing}/{len(brawlers)}",
            )